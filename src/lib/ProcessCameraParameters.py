...
class CameraTransformer:
    ...
    def project_points_3d_to_2d(self, x):
        return project_points_3d_to_2d(
            x,
            *get_anno(self.record, *self.parameters_3d_to_2d.split(", ")),
        )

    def project_points_2d_to_3d(self, x):
        return project_points_2d_to_3d(
            x,
            *get_anno(self.record, *self.parameters_2d_to_3d.split(", ")),
        )
...
def get_anno_ndarray(record, *args, idx=0):
    ...
    for key_ in args:
        if key_ == "height":
            ...
            out.append(
                float(record["objects"][0, 0]["viewpoint"][0, idx][key_][0, 0][0, 0]),
            )
